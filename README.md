# Digital Marketing Graduation Project – Real Client

## Project Overview
This project is a graduation requirement for a Digital Marketing course.
The project focuses on managing social media accounts for a real client to apply practical digital marketing strategies in a real-world scenario.

## Project Type
Team Project (5 Members)

## Project Objective
- Apply digital marketing concepts in a real business environment
- Build and execute a structured social media marketing strategy
- Analyze performance and measure results
- Gain hands-on experience working with a real client

## Project Scope
The project includes:
- Social media account analysis
- Content planning and scheduling
- Brand positioning and tone of voice definition
- Performance tracking and reporting

## Platforms
(To be determined)

## Client Information
The client is a real business.
Client details will be finalized during the project execution phase.

## Team Roles
Roles and responsibilities will be assigned and documented during the planning phase.

## Project Status
Planning Phase
